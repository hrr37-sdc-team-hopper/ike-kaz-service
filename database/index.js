...
//   return query('SELECT id, id as bookInfo_id, image FROM bookInfo WHERE id=' + bookId);
// };
...
//   return query('SELECT id, email FROM users WHERE id=' + bookInfo_id);
// };
...
//   return query(
//     'SELECT id, users.bookInfo_id, users.id as user_id, num_rating FROM bookInfo JOIN users ON users.id = bookInfo.id WHERE id=' +
//       bookInfo_id
//   );
// };

// // const insertRatings = function(bookId, userId, rating) {
// //   return query('INSERT INTO ratings (bookInfo_id, user_id, rating) VALUES(?, ?, ?)', [
// //     bookId,
// //     userId,
// //     rating
// //   ]);
...
//   return query('SELECT id, id as bookInfo, description FROM bookInfo WHERE id=' + bookInfo_id);
// };
...
//   return query(
//     `UPDATE readStatus SET status = ${status} WHERE user_id = ${userId} AND bookInfo_id = ${bookId}`
//   );
// };

// const insertReadStatus = function(bookId, userId, status) {
//   return query(`INSERT INTO readStatus (status, user_id, bookInfo_id) VALUES(?,?,?)`, [
//     status,
//     userId,
//     bookId
//   ]);
...
// image, ratings, reviews, readStatus, shelf, bookShelf
const insertBookInfo = function(data) {
  return query('INSERT INTO bookInfo (title, author, description) VALUES(?, ?, ?)', [
    data.title,
    data.author,
    data.description
  ]);
...
const insertUsers = function(user) {
  return query('INSERT INTO users (email, bookInfo_id) VALUES(?, ?)', [
    user.email,
    user.bookInfo_id
  ]);
};
